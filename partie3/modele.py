...
class Trajet(Affichable):
    ...
    def __repr__(self):
        return f"Tr:{self.depart}/{self.arrivee}/{self.distance}"
    ...
    def __eq__(self, other):
        if isinstance(other,Trajet):
            return (self.depart == other.depart
                    and self.arrivee == other.arrivee)
    ...
    def __add__(self, other):
        ...
        if isinstance(other, (int)):
            return Trajet(self.depart, self.arrivee, self.distance + other)
    ...
    # utilse pour les set , les dictionnaires : RAPIDE
    def __hash__(self):
        return hash((self.depart, self.arrivee, self.distance))

    def __lt__(self, other):
        if isinstance(other,Trajet):
            if self.depart == other.depart:
                return self.arrivee < other.arrivee
            return self.depart < other.depart
        elif isinstance(other,(int,float)):
            return self.distance < other
        return False
...
class SystemeReservation:
    ...
    def __del__(self):
        choix = input("Voulez-vous effacer json").lower()
        if choix in ["oui","y","yes","o"]:
            print("OK j'efface")

    def __getitem__(self, cle):
        users = [u for u in self.utilisateurs if u.nom == cle]
        return users[0] if users else None


if __name__ == "__main__":
    t = Trajet("Limoges", "Lyon", 460)
    print(t.to_dict())